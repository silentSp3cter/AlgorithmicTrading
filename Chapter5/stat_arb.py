...
# print(normalized_price_data.head())
# normalized_price_data.info()
...
for i in range(0, num_days):
    ...
    for ticker in normalized_price_data.columns:
        ...
        if len(price_deviation_from_sma[ticker]) < 2:  # need atleast two observations to compute covariance/correlation
    ...
    if ((final_delta_projected < StatArb_VALUE_FOR_SELL_ENTRY and abs(
            close_price - last_sell_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE)  # StatArb above sell entry threshold,
            # we should sell
            or
            (position > 0 and (
                    open_pnl > MIN_PROFIT_TO_CLOSE))):  # long from -ve StatArb and StatArb has gone positive or
    ...
    # 2. We are short( -ve position ) and current position is profitable enough to lock profit.
            ...
            close_price - last_buy_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE)  # StatArb below buy entry threshold,
          # we should buy
          or
          (position < 0 and (
                  open_pnl > MIN_PROFIT_TO_CLOSE))):  # short from +ve StatArb and StatArb has gone negative or position
