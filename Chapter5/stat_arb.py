...
TRADING_INSTRUMENT = 'NIFTY BANK'
tickers = ['NIFTY 50', 'NIFTY BANK', 'NIFTY AUTO', 'NIFTY FIN SERVICE', 'NIFTY FMCG', 'NIFTY IT', 'NIFTY MEDIA',
           'NIFTY METAL', 'NIFTY PHARMA', 'NIFTY PSU BANK', 'NIFTY PVT BANK', 'NIFTY REALTY']
tickers_data = []
cycol = cycle('bgrcmky')
for ticker in tickers:
    ticker_data = get_history(ticker, start=date(2014, 1, 1), end=date(2018, 1, 1), index=True)
    tickers_data.append(ticker_data)

normalized_price_data = pd.DataFrame()

for i in range(1, len(tickers)):
    ticker_price = tickers_data[i]["Close"]
    nifty50_price = tickers_data[0]["Close"]
    normalized_price = ticker_price / nifty50_price
    normalized_price_data[tickers[i]] = normalized_price

normalized_price_data.index = pd.to_datetime(normalized_price_data.index)

# Function returns final pnl value for the target instrument that is passed.
def calculateProfit(ticker,StatArb_VALUE_FOR_BUY_ENTRY,SMA_NUM_PERIODS):
    StatArb_VALUE_FOR_SELL_ENTRY = - StatArb_VALUE_FOR_BUY_ENTRY
    TRADING_INSTRUMENT = ticker
    ...
    num_days = len(normalized_price_data[TRADING_INSTRUMENT].index)
    correlation_history = {}  # history of correlations per currency pair
    delta_projected_actual_history = {}  # history of differences between Projected ClosePrice deviation and
    # actual ClosePrice deviation per currency pair

    final_delta_projected_history = []  # history of differences between final Projected ClosePrice deviation for
    # TRADING_INSTRUMENT and actual ClosePrice deviation
    ...
    positions = []  # Container for tracking positions, +ve for long positions, -ve for short positions, 0 for
    # flat/no position
    pnls = []  # Container for tracking total_pnls, this is the sum of closed_pnl i.e. pnls already locked in and
    ...
    buy_sum_price_qty = 0  # Summation of products of buy_trade_price and buy_trade_qty for every buy Trade made since
    # last time being flat
    buy_sum_qty = 0  # Summation of buy_trade_qty for every buy Trade made since last time being flat
    sell_sum_price_qty = 0  # Summation of products of sell_trade_price and sell_trade_qty for every sell Trade made
    ...
    MIN_PRICE_MOVE_FROM_LAST_TRADE = 0.01  # Minimum price change since last trade before considering trading again,
    # this is to prevent over-trading at/around same prices
    NUM_SHARES_PER_TRADE = 1  # Number of currency to buy/sell on every trade
    ...
    for i in range(0, num_days):
        ...
        for ticker in normalized_price_data.columns:
            close_prices[ticker] = normalized_price_data[ticker].iloc[i]
            if ticker not in price_history.keys():
                price_history[ticker] = []
                price_deviation_from_sma[ticker] = []

            price_history[ticker].append(close_prices[ticker])
            if len(price_history[ticker]) > SMA_NUM_PERIODS:  # we track at most SMA_NUM_PERIODS number of prices
                del (price_history[ticker][0])

            sma = stats.mean(price_history[ticker])  # Rolling SimpleMovingAverage
            price_deviation_from_sma[ticker].append(close_prices[ticker] - sma)  # price deviation from mean
        ...
        for ticker in normalized_price_data.columns:
            if ticker == TRADING_INSTRUMENT:  # no need to find relationship between trading instrument and itself
                continue
            ...
            if len(price_deviation_from_sma[
                       ticker]) < 2:  # need atleast two observations to compute covariance/correlation
            ...
            projected_dev_from_sma_using[ticker] = price_deviation_from_sma[ticker][
            ...
            delta_projected_actual = (
                    projected_dev_from_sma_using[ticker] - price_deviation_from_sma[TRADING_INSTRUMENT][-1])
        ...
        final_delta_projected = 0  # will hold final prediction of price deviation in TRADING_INSTRUMENT,
        ...
        # 1. The StatArb trading signal value is below Sell-Entry threshold and the difference between last trade-price
        # and current-price is different enough.
        # 2. We are long( +ve position ) and current position is profitable enough to lock profit.

        if final_delta_projected < StatArb_VALUE_FOR_SELL_ENTRY : # StatArb above sell entry threshold,
                # we should sell  # long from -ve StatArb and StatArb has gone positive or
            # position is profitable, sell to close position
        ...
        # 1. The StatArb trading signal value is above Buy-Entry threshold and the difference between last trade-price
        # and current-price is different enough.
        # 2. We are short( -ve position ) and current position is profitable enough to lock profit.

        elif final_delta_projected > StatArb_VALUE_FOR_BUY_ENTRY:  # StatArb below buy entry threshold,
              # we should buy
            # is profitable, buy to close position
        ...
        open_pnl = 0
        if position > 0:
            if sell_sum_qty > 0:  # long position and some sell trades have been made against it, close that amount based
                # on how much was sold against this long position
            ...
            if buy_sum_qty > 0:  # short position and some buy trades have been made against it, close that amount based
                # on how much was bought against this short position
...
for StatArb_VALUE_FOR_BUY_ENTRY in [0.005, 0.01, 0.015, 0.020]:
    StatArb_VALUE_FOR_SELL_ENTRY = - StatArb_VALUE_FOR_BUY_ENTRY
    for SMA_NUM_PERIODS in [20, 40, 60]:
        df_index = '{' + str(StatArb_VALUE_FOR_SELL_ENTRY) + ',' + str(StatArb_VALUE_FOR_BUY_ENTRY) + ',' + str(SMA_NUM_PERIODS) + '}'
        ticker_profits = []  # Stores final pnl value for each target ticker and a given parameter set
        for ticker in tickers[1:]:

            profit = calculateProfit(ticker, StatArb_VALUE_FOR_BUY_ENTRY, SMA_NUM_PERIODS);

            ticker_profits.append(profit)

        df.loc[df_index] = ticker_profits
...
# Commenting out rest of the plotting code for now.
'''
...
for ticker in normalized_price_data.columns:
    ...
    correlation_data = correlation_data.assign(label=pd.Series(correlation_history[correlation_label],
                                                               index=normalized_price_data[ticker].index))
...
for ticker in normalized_price_data.columns:
    ...
    delta_projected_actual_data = delta_projected_actual_data.assign(
        StatArbTradingSignal=pd.Series(delta_projected_actual_history[projection_label],
                                       index=normalized_price_data[TRADING_INSTRUMENT].index))
...
delta_projected_actual_data = delta_projected_actual_data.assign(ClosePrice=pd.Series(
    normalized_price_data[TRADING_INSTRUMENT], index=normalized_price_data[TRADING_INSTRUMENT].index))
delta_projected_actual_data = delta_projected_actual_data.assign(
    FinalStatArbTradingSignal=pd.Series(final_delta_projected_history,
                                        index=normalized_price_data[TRADING_INSTRUMENT].index))
delta_projected_actual_data = delta_projected_actual_data.assign(
    Trades=pd.Series(orders, index=normalized_price_data[TRADING_INSTRUMENT].index))
delta_projected_actual_data = delta_projected_actual_data.assign(
    Position=pd.Series(positions, index=normalized_price_data[TRADING_INSTRUMENT].index))
delta_projected_actual_data = delta_projected_actual_data.assign(
    Pnl=pd.Series(pnls, index=normalized_price_data[TRADING_INSTRUMENT].index))

plt.plot(delta_projected_actual_data.index, delta_projected_actual_data.ClosePrice,
